<template>
  <div class="min-h-screen mt-[70px]">
    <nav class="container mx-auto px-4 pt-4 text-sm text-gray-500 dark:text-gray-400">
      ...
      <span class="mx-2">/</span>
      <span>Side Shifter</span>
    </nav>

    <section class="relative py-20 px-4 bg-gray-50 dark:bg-zinc-900/50 mt-4">
      <div class="max-w-4xl mx-auto text-center">
        <h1 class="text-4xl md:text-5xl font-bold leading-tight mb-6">
          Side Shifter on Rent in
          <span class="text-[#FF4057]">Navi Mumbai</span> | 20T to 40T Container Handler
        </h1>
        <p class="text-lg text-gray-600 dark:text-gray-300 mb-8 max-w-3xl mx-auto">
          Saini Lifters provides Kalmar side shifters on rent in Navi Mumbai and Kalamboli.
          Our 20T and 40T side-shifting container handlers reposition containers laterally with precision —
          essential for container freight stations, export cargo handling and tight yard operations near JNPT.
        </p>
    ...
    <section class="max-w-5xl mx-auto px-4 py-12">
      <div class="w-full rounded-xl overflow-hidden mb-12 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700">
        <img src="/image/eq-side-shifter.png" alt="Kalmar side shifter container handler on rent in Navi Mumbai" class="w-full h-auto object-contain max-h-[480px] mx-auto p-4" loading="eager" />
      </div>

      <h2 class="text-3xl font-bold mb-8">Side Shifter Specifications</h2>
      <div class="grid md:grid-cols-2 gap-6 mb-12">
        <div v-for="item in specs" :key="item.name" class="border border-gray-200 dark:border-zinc-700 rounded-xl p-6 hover:border-[#FF4057] transition-colors">
          <h3 class="font-bold text-xl mb-2 text-[#FF4057]">{{ item.name }}</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">{{ item.desc }}</p>
          <div class="grid grid-cols-2 gap-3">
            <div v-for="s in item.data" :key="s.label" class="text-sm">
              <span class="text-gray-400 block text-xs uppercase tracking-wider">{{ s.label }}</span>
    ...
    <section class="py-16 bg-[#FF4057]/5 dark:bg-[#FF4057]/10">
      <div class="max-w-3xl mx-auto text-center px-4">
        <h2 class="text-3xl font-bold mb-4">Book Side Shifter in Navi Mumbai</h2>
        <p class="text-gray-500 dark:text-gray-300 mb-6">
          Also see:
          <NuxtLink to="/services/side-shifter-rental" class="text-[#FF4057] hover:underline">Side Shifter Service</NuxtLink> |
          <NuxtLink to="/equipment/reach-stacker" class="text-[#FF4057] hover:underline">Reach Stacker</NuxtLink> |
          <NuxtLink to="/crane-rental/kalamboli" class="text-[#FF4057] hover:underline">Crane Rental Kalamboli</NuxtLink>
        </p>
        <div class="flex flex-wrap gap-4 justify-center">
          <a href="tel:+919322296990" class="bg-[#FF4057] hover:bg-red-600 text-white px-8 py-3 rounded-lg font-semibold transition-colors">Call: +91 9322296990</a>
          <a href="https://wa.me/919322296990?text=Hi%20I%20need%20side%20shifter%20on%20rent%20in%20Navi%20Mumbai" class="border-2 border-[#FF4057] text-[#FF4057] hover:bg-[#FF4057] hover:text-white px-8 py-3 rounded-lg font-semibold transition-colors">WhatsApp Us</a>
        </div>
      </div>
    </section>
  </div>
...
useHead({
  title: "Side Shifter on Rent in Navi Mumbai | 20T to 40T Container Handler | Saini Lifters",
  meta: [
    { name: "description", content: "Side shifter container handler on rent in Navi Mumbai & Kalamboli — Kalmar 20T and 40T for CFS, export cargo and container repositioning near JNPT. Call +91 9322296990." },
    { name: "robots", content: "index, follow" },
    { name: "keywords", content: "side shifter on rent navi mumbai, container handler navi mumbai, side shifter rental JNPT, kalmar side shifter navi mumbai" },
    { property: "og:url", content: "https://www.sainilifters.com/equipment/side-shifter" },
    { property: "og:title", content: "Side Shifter on Rent in Navi Mumbai | 20T to 40T Container Handler" },
    { property: "og:description", content: "Side shifter on rent in Navi Mumbai — Kalmar 20T & 40T for CFS, export cargo. Call +91 9322296990." },
    { property: "og:image", content: "https://www.sainilifters.com/image/eq-side-shifter.png" },
  ],
  link: [{ rel: "canonical", href: "https://www.sainilifters.com/equipment/side-shifter" }],
  script: [
...
const specs = [
  { name: "Kalmar 20T Side Shifter", desc: "Precise lateral container positioning for 20ft ISO containers in CFS and export cargo operations.", data: [{ label: "Capacity", value: "20T" }, { label: "Container Size", value: "20ft ISO" }, { label: "Shift Range", value: "±600mm" }, { label: "Operation", value: "CFS / Export" }] },
  { name: "Kalmar 40T Side Shifter", desc: "Heavy-duty side shifting for 40ft ISO containers — export cargo, reefer handling and container terminal fine placement.", data: [{ label: "Capacity", value: "40T" }, { label: "Container Size", value: "20ft & 40ft" }, { label: "Shift Range", value: "±600mm" }, { label: "Operation", value: "Terminal / CFS" }] },
];

const applications = [
  "Container freight station (CFS) operations",
  "Export cargo container positioning",
  "Reefer (refrigerated) container handling",
  "Container terminal fine placement",
  "Tight yard operations near JNPT",
  "Container row repositioning",
  "ISO container loading on flatbeds",
